...
object JsonParserSpec extends Specification {
  ...
  "A JSON response with headers and content elements" should {
    ...
    "parse to equivalent Response" in {
      val expected = Response(Map("aa" -> "bb", "cc" -> "dd"), Some(TreeMap("b" -> 2)))
      val parsed = JsonParser.parse(jsonResponse).get
      parsed must equalTo(expected)
    }
  }
  "Request generated by JsonFormatter" should {
    val request = Request(Map("key" -> "value"), Some(TreeMap("a" -> 1)))
    val jsonRequest = JsonFormatter.format(request)
    "must be properly parsed to Response by JsonParser" in {
      val expected = Response(Map("key" -> "value"), Some(TreeMap("a" -> 1)))
      val parsed = JsonParser.parse(jsonRequest).get
      parsed must equalTo(expected)
    }
  }
}
